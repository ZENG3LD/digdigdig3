...
/// 24 B record (LE):
///   i64 ts_ms (8), f64 volatility (8), f64 _pad (8, NaN)
...
impl DataPoint for HistoricalVolatilityPoint {
    const RECORD_SIZE: usize = SIZE;

    fn encode(&self, out: &mut [u8]) {
        out[0..8].copy_from_slice(&(self.ts_ms as u64).to_le_bytes());
        out[8..16].copy_from_slice(&self.volatility.to_le_bytes());
    ...
    fn decode(bytes: &[u8]) -> Option<Self> {
        ...
        Some(Self {
            ...
            volatility: f64::from_le_bytes(bytes[8..16].try_into().ok()?),
        })
    }
    ...
    fn from_stream_event(ev: &StreamEvent) -> Option<Self> {
        if let StreamEvent::HistoricalVolatility { symbol: _, volatility, timestamp } = ev {
            Some(Self { ts_ms: *timestamp, volatility: *volatility })
        } else {
            None
        }
    }
