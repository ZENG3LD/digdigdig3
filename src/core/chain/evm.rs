...
mod tests {
    ...
    #[test]
    fn test_evm_provider_chain_family() {
        let provider = EvmProvider::arbitrum();
        assert_eq!(provider.chain_family(), ChainFamily::Evm { chain_id: 42161 });
        assert_eq!(provider.chain_name, "arbitrum");
    }

    #[test]
    fn test_evm_provider_known_chains() {
        let arb = EvmProvider::arbitrum();
        assert_eq!(arb.chain_id, 42161);

        let base = EvmProvider::base();
        assert_eq!(base.chain_id, 8453);

        let poly = EvmProvider::polygon();
        assert_eq!(poly.chain_id, 137);

        let avax = EvmProvider::avalanche();
        assert_eq!(avax.chain_id, 43114);

        let bsc = EvmProvider::bsc();
        assert_eq!(bsc.chain_id, 56);

        let opt = EvmProvider::optimism();
    ...
    #[test]
    fn test_erc20_calldata_encoding() {
        // Verify that the calldata structure is correct:
        // 4 bytes selector + 12 zero bytes + 20 bytes address = 36 bytes
        let token: Address = "0xA0b86991c6218b36c1d19D4a2e9Eb0cE3606eB48".parse().unwrap();
        let account: Address = "0x742d35Cc6634C0532925a3b8D4C9C42D9b1e7C5a".parse().unwrap();

        // Replicate the encoding to check length
        let mut calldata = Vec::with_capacity(4 + 32);
        calldata.extend_from_slice(&BALANCE_OF_SELECTOR);
        calldata.extend_from_slice(&[0u8; 12]);
        calldata.extend_from_slice(account.as_slice());
        ...
        // token is only used as `to` address in the call, not in calldata
        let _ = token;
    }
}
