...
//!
//! Tests WebSocket connectivity and subscriptions against real Bitget API.
//!
//! Run with:
//! ```text
//! cargo test --package digdigdig3 --lib crypto::cex::bitget::_tests_websocket -- --ignored --nocapture
//! ```
//!
//! NOTE: All tests connect to real Bitget endpoints and require network access.
...
async fn test_orderbook_capabilities() {
    ...
    println!("Bitget orderbook capabilities: {:?}", caps);
...
async fn test_subscribe_orderbook() {
    ...
    match connect_result {
        Ok(Ok(())) => {
            ...
            println!("Subscribed to Bitget orderbook — waiting for snapshot...");
            ...
            }).await.expect("Timeout waiting for orderbook data — Bitget did not send snapshot/delta within 15s");
            ...
            let _ = ws.disconnect().await;
            println!("Bitget orderbook subscription works");
        }
...
async fn test_subscribe_trades() {
    ...
    match connect_result {
        Ok(Ok(())) => {
            ...
            println!("Subscribed to Bitget trades — waiting for trade...");
            ...
            let _ = ws.disconnect().await;
            println!("Bitget trades subscription works");
        }
        Ok(Err(e)) => println!("Connection failed: {:?}", e),
        Err(_) => println!("Connection timeout"),
    }
