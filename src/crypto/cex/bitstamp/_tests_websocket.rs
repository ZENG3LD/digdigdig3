//! Bitstamp WebSocket Integration Tests
//!
//! Tests WebSocket connectivity and subscriptions against real Bitstamp API.
//! Bitstamp uses Pusher protocol over WebSocket.
//!
//! Run with:
//! ```text
//! cargo test --package digdigdig3 --lib crypto::cex::bitstamp::_tests_websocket -- --ignored --nocapture
//! ```
//!
//! NOTE: All tests connect to real Bitstamp endpoints and require network access.
//! Bitstamp orderbook does not support a configurable depth parameter.
...
use super::websocket::BitstampWebSocket;
...
fn btc_usd() -> Symbol {
    // Bitstamp uses lowercase btcusd format (handled internally)
    Symbol::new("BTC", "USD")
}
...
async fn test_orderbook_capabilities() {
    let ws = match BitstampWebSocket::new().await {
        Ok(w) => w,
        Err(e) => {
    ...
    println!("Bitstamp orderbook capabilities: {:?}", caps);
...
async fn test_subscribe_orderbook() {
    ...
    match connect_result {
        Ok(Ok(())) => {
            ...
            // Bitstamp does not use depth parameter
            ...
            println!("Subscribed to Bitstamp orderbook (order_book_btcusd) — waiting for snapshot...");
            ...
            }).await.expect("Timeout waiting for orderbook data — Bitstamp did not send snapshot/delta within 15s");
            ...
            let _ = ws.disconnect().await;
            println!("Bitstamp orderbook subscription works");
        }
...
async fn test_subscribe_trades() {
    ...
    match connect_result {
        Ok(Ok(())) => {
            ...
            println!("Subscribed to Bitstamp trades (live_trades_btcusd) — waiting for trade...");
            ...
            let _ = ws.disconnect().await;
            println!("Bitstamp trades subscription works");
        }
        Ok(Err(e)) => println!("Connection failed: {:?}", e),
        Err(_) => println!("Connection timeout"),
    }
