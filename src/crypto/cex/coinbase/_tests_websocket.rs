//! Coinbase WebSocket Integration Tests
//!
//! Tests WebSocket connectivity and subscriptions against real Coinbase Advanced Trade API.
//!
//! Run with:
//! ```text
//! cargo test --package digdigdig3 --lib crypto::cex::coinbase::_tests_websocket -- --ignored --nocapture
//! ```
//!
//! NOTE: All tests connect to real Coinbase endpoints and require network access.
//! Coinbase orderbook does not support a configurable depth parameter.
...
use super::websocket::CoinbaseWebSocket;
...
async fn test_orderbook_capabilities() {
    let ws = match CoinbaseWebSocket::new(None).await {
        Ok(w) => w,
        Err(e) => {
    ...
    println!("Coinbase orderbook capabilities: {:?}", caps);
...
async fn test_subscribe_orderbook() {
    let mut ws = match CoinbaseWebSocket::new(None).await {
        Ok(w) => w,
        Err(e) => {
            println!("Failed to create WebSocket: {:?}", e);
            return;
    ...
    match connect_result {
        Ok(Ok(())) => {
            ...
            // Coinbase does not use depth parameter
            ...
            println!("Subscribed to Coinbase orderbook — waiting for snapshot...");
            ...
            }).await.expect("Timeout waiting for orderbook data — Coinbase did not send snapshot/delta within 15s");
            ...
            let _ = ws.disconnect().await;
            println!("Coinbase orderbook subscription works");
        }
...
async fn test_subscribe_trades() {
    ...
    match connect_result {
        Ok(Ok(())) => {
            ...
            println!("Subscribed to Coinbase trades — waiting for trade...");
            ...
            let _ = ws.disconnect().await;
            println!("Coinbase trades subscription works");
        }
        Ok(Err(e)) => println!("Connection failed: {:?}", e),
        Err(_) => println!("Connection timeout"),
    }
