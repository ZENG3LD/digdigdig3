//! Deribit WebSocket Integration Tests
//!
//! Tests WebSocket connectivity and subscriptions against real Deribit API.
//! Deribit uses JSON-RPC 2.0 over WebSocket. Symbol: BTC adds -PERPETUAL internally.
//!
//! Run with:
//! ```text
//! cargo test --package digdigdig3 --lib crypto::cex::deribit::_tests_websocket -- --ignored --nocapture
//! ```
//!
//! NOTE: All tests connect to real Deribit endpoints and require network access.
...
use super::websocket::DeribitWebSocket;
...
fn btc() -> Symbol {
    // Deribit uses BTC — connector internally formats as BTC-PERPETUAL
    Symbol::new("BTC", "USD")
}
...
async fn test_orderbook_capabilities() {
    ...
    println!("Deribit orderbook capabilities: {:?}", caps);
...
async fn test_subscribe_orderbook() {
    ...
    match connect_result {
        Ok(Ok(())) => {
            ...
            let sub = SubscriptionRequest::new(btc(), StreamType::Orderbook);
            ...
            println!("Subscribed to Deribit orderbook (BTC-PERPETUAL) — waiting for snapshot...");
            ...
            }).await.expect("Timeout waiting for orderbook data — Deribit did not send snapshot/delta within 15s");
            ...
            let _ = ws.disconnect().await;
            println!("Deribit orderbook subscription works");
        }
...
async fn test_subscribe_trades() {
    ...
    match connect_result {
        Ok(Ok(())) => {
            let sub = SubscriptionRequest::new(btc(), StreamType::Trade);
            let result = ws.subscribe(sub).await;
            ...
            println!("Subscribed to Deribit trades (BTC-PERPETUAL) — waiting for trade...");
            ...
            let _ = ws.disconnect().await;
            println!("Deribit trades subscription works");
        }
        Ok(Err(e)) => println!("Connection failed: {:?}", e),
        Err(_) => println!("Connection timeout"),
    }
