//! HyperLiquid WebSocket Integration Tests
//!
//! Tests WebSocket connectivity and subscriptions against real HyperLiquid API.
//! HyperLiquid is a DEX — symbol is the coin name (e.g., "BTC"), no depth param.
//!
//! Run with:
//! ```text
//! cargo test --package digdigdig3 --lib crypto::cex::hyperliquid::_tests_websocket -- --ignored --nocapture
//! ```
//!
//! NOTE: All tests connect to real HyperLiquid endpoints and require network access.
...
use super::websocket::HyperliquidWebSocket;
...
fn btc() -> Symbol {
    // HyperLiquid uses coin name directly: "BTC" (perpetual)
    Symbol::new("BTC", "USD")
}
...
async fn test_subscribe_orderbook() {
    ...
    match connect_result {
        Ok(Ok(())) => {
            ...
            // HyperLiquid does not use depth parameter
            ...
            println!("Subscribed to HyperLiquid orderbook (BTC) — waiting for snapshot...");
            ...
            }).await.expect("Timeout waiting for orderbook data — HyperLiquid did not send snapshot/delta within 15s");
            ...
            let _ = ws.disconnect().await;
            println!("HyperLiquid orderbook subscription works");
        }
...
async fn test_subscribe_trades() {
    ...
    match connect_result {
        Ok(Ok(())) => {
            ...
            println!("Subscribed to HyperLiquid trades (BTC) — waiting for trade...");
            ...
            let _ = ws.disconnect().await;
            println!("HyperLiquid trades subscription works");
        }
        Ok(Err(e)) => println!("Connection failed: {:?}", e),
        Err(_) => println!("Connection timeout"),
    }
