//! Phemex WebSocket Integration Tests
//!
//! Tests WebSocket connectivity and subscriptions against real Phemex API.
//! Phemex requires heartbeat every 5 seconds.
//!
//! Run with:
//! ```text
//! cargo test --package digdigdig3 --lib crypto::cex::phemex::_tests_websocket -- --ignored --nocapture
//! ```
//!
//! NOTE: All tests connect to real Phemex endpoints and require network access.
//! Phemex orderbook does not support a configurable depth parameter.
//! Symbol is BTCUSD (inverse perpetual).
...
use super::websocket::PhemexWebSocket;
...
fn btc_usd() -> Symbol {
    // Phemex uses BTCUSD for inverse perpetual (no depth param)
    Symbol::new("BTC", "USD")
}
...
async fn test_orderbook_capabilities() {
    let ws = match PhemexWebSocket::new(None, false).await {
        Ok(w) => w,
        Err(e) => {
    ...
    println!("Phemex orderbook capabilities: {:?}", caps);
...
async fn test_subscribe_orderbook() {
    let mut ws = match PhemexWebSocket::new(None, false).await {
        Ok(w) => w,
        Err(e) => {
            println!("Failed to create WebSocket: {:?}", e);
            return;
    ...
    // Phemex is contract-based — use FuturesCross account type
    ...
    match connect_result {
        Ok(Ok(())) => {
            ...
            // Phemex does not use depth parameter
            ...
            println!("Subscribed to Phemex orderbook (BTCUSD) — waiting for snapshot...");
            ...
            }).await.expect("Timeout waiting for orderbook data — Phemex did not send snapshot/delta within 15s");
            ...
            let _ = ws.disconnect().await;
            println!("Phemex orderbook subscription works");
        }
...
async fn test_subscribe_trades() {
    ...
    match connect_result {
        Ok(Ok(())) => {
            ...
            println!("Subscribed to Phemex trades — waiting for trade...");
            ...
            let _ = ws.disconnect().await;
            println!("Phemex trades subscription works");
        }
        Ok(Err(e)) => println!("Connection failed: {:?}", e),
        Err(_) => println!("Connection timeout"),
    }
