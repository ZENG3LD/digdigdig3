...
mod tests {
    ...
    async fn test_create_websocket() {
        let ws = PhemexWebSocket::new(None, false).await;
        assert!(ws.is_ok());
    }

    #[tokio::test]
    async fn test_create_websocket_testnet() {
        let ws = PhemexWebSocket::new(None, true).await;
        assert!(ws.is_ok());
    }

    #[tokio::test]
    async fn test_connection_status_disconnected() {
        let ws = PhemexWebSocket::new(None, false).await.unwrap();
    ...
    #[test]
    fn test_build_subscribe_trade() {
        let request = SubscriptionRequest::trade(Symbol::new("BTC", "USD"));
        let (method, params) =
            PhemexWebSocket::build_subscribe_message(&request, AccountType::FuturesCross);
        assert_eq!(method, "trade.subscribe");
        assert_eq!(params.len(), 1);
        assert_eq!(params[0].as_str().unwrap(), "BTCUSD");
    }

    #[test]
    fn test_build_subscribe_orderbook() {
        let request = SubscriptionRequest::orderbook(Symbol::new("BTC", "USD"));
        let (method, params) =
            PhemexWebSocket::build_subscribe_message(&request, AccountType::FuturesCross);
        assert_eq!(method, "orderbook.subscribe");
        assert_eq!(params.len(), 1);
        assert_eq!(params[0].as_str().unwrap(), "BTCUSD");
    }
    ...
    fn test_build_subscribe_ticker() {
        let request = SubscriptionRequest::ticker(Symbol::new("BTC", "USD"));
        let (method, _params) =
            PhemexWebSocket::build_subscribe_message(&request, AccountType::FuturesCross);
        assert_eq!(method, "market24h.subscribe");
    }

    #[test]
    fn test_build_subscribe_kline() {
        let request = SubscriptionRequest::kline(Symbol::new("BTC", "USD"), "1m");
        let (method, params) =
            PhemexWebSocket::build_subscribe_message(&request, AccountType::FuturesCross);
        assert_eq!(method, "kline.subscribe");
        assert_eq!(params.len(), 2);
        assert_eq!(params[0].as_str().unwrap(), "BTCUSD");
        assert_eq!(params[1].as_u64().unwrap(), 60);
    }

    #[test]
    fn test_build_subscribe_spot_symbol() {
        let request = SubscriptionRequest::trade(Symbol::new("BTC", "USDT"));
        let (method, params) =
            PhemexWebSocket::build_subscribe_message(&request, AccountType::Spot);
        assert_eq!(method, "trade.subscribe");
        assert_eq!(params[0].as_str().unwrap(), "sBTCUSDT");
    }

    #[test]
    fn test_build_unsubscribe_methods() {
        assert_eq!(
            PhemexWebSocket::build_unsubscribe_method(&StreamType::Ticker),
            "market24h.unsubscribe"
        );
        assert_eq!(
            PhemexWebSocket::build_unsubscribe_method(&StreamType::Trade),
            "trade.unsubscribe"
        );
        assert_eq!(
            PhemexWebSocket::build_unsubscribe_method(&StreamType::Orderbook),
            "orderbook.unsubscribe"
        );
        assert_eq!(
            PhemexWebSocket::build_unsubscribe_method(&StreamType::OrderUpdate),
            "aop.unsubscribe"
        );
    }
    ...
    fn testis_private() {
        ...
        assert!(PhemexWebSocket::is_private(&StreamType::OrderUpdate));
        assert!(PhemexWebSocket::is_private(&StreamType::BalanceUpdate));
        assert!(PhemexWebSocket::is_private(&StreamType::PositionUpdate));
    }
    ...
    fn test_parse_orderbook_snapshot() {
        ...
        let result = PhemexWebSocket::parse_push_message(&msg, 4).unwrap();
        assert!(result.is_some());

        if let Some(StreamEvent::OrderbookSnapshot(ob)) = result {
            assert_eq!(ob.bids.len(), 2);
            assert_eq!(ob.asks.len(), 2);
            assert!((ob.bids[0].0 - 8770.0).abs() < 0.1);
            assert!((ob.asks[0].0 - 8770.5).abs() < 0.1);
        } else {
    ...
    #[test]
    fn test_parse_orderbook_incremental() {
        let msg = json!({
            "book": {
                "asks": [[87710000i64, 500000]],
                "bids": [[87695000i64, 0]]
            },
            "sequence": 123456790i64,
            "timestamp": 1234567890100000000i64,
            "symbol": "BTCUSD",
            "type": "incremental"
        });
        ...
        if let Some(StreamEvent::OrderbookDelta { bids, asks, .. }) = result {
    ...
    fn test_parse_trades() {
        let msg = json!({
            "trades": [
                [1234567890000000000i64, "Buy", 87705000i64, 1000],
                [1234567891000000000i64, "Sell", 87700000i64, 500]
            ],
            "sequence": 123456789i64,
            "symbol": "BTCUSD",
            "type": "snapshot"
        ...
        if let Some(StreamEvent::Trade(trade)) = result {
            ...
            assert!((trade.price - 8770.0).abs() < 0.1);
    ...
    fn test_parse_kline() {
        let msg = json!({
            "kline": [
                [1590019200i64, 60, 87700000i64, 87750000i64, 87650000i64, 87680000i64, 123456, 1234567890i64]
            ],
            "sequence": 123456789i64,
            "symbol": "BTCUSD",
            "type": "snapshot"
        ...
        if let Some(StreamEvent::Kline(kline)) = result {
            assert!((kline.open - 8768.0).abs() < 0.1);
            assert!((kline.close - 8770.0).abs() < 0.1);
            assert!((kline.high - 8775.0).abs() < 0.1);
            assert!((kline.low - 8765.0).abs() < 0.1);
        } else {
