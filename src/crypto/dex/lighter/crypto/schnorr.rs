...
mod tests {
    use super::*;
    use super::super::poseidon2::hash_to_quintic_extension;
    ...
    fn test_sign_verify_roundtrip() {
        ...
        // Hash a test message
        let msg_elements = [
            GFp::from_canonical_u64(304),   // chain_id
            GFp::from_canonical_u64(14),    // tx_type
            GFp::from_canonical_u64(1),     // nonce
        ];
        let msg_hash = hash_to_quintic_extension(&msg_elements);

        // Sign
        let sig_bytes = sign(&sk_bytes, msg_hash);

        // Verify
        let hash_bytes: [u8; 40] = msg_hash.encode();
        let valid = verify(&pk_bytes, msg_hash, &sig_bytes);
    ...
    fn test_sign_verify_wrong_key() {
        ...
        let msg_hash = GFp5::from_u64_reduce(42, 1, 2, 3, 4);
        let sig = sign(&sk1, msg_hash);

        let valid = verify(&pk2, msg_hash, &sig);
        assert!(!valid, "signature with wrong key should not verify");
    }

    #[test]
    fn test_derive_public_key_deterministic() {
        let sk: [u8; 40] = [7; 40];
        let pk1 = derive_public_key(&sk);
        let pk2 = derive_public_key(&sk);
        assert_eq!(pk1, pk2, "key derivation is deterministic");
    }
}
