...
//!
//! Etherscan uses simple API key authentication via query parameter.
//! No HMAC signatures, no headers - just append `apikey=YOUR_KEY` to the URL.

use std::collections::HashMap;

/// Etherscan authentication credentials
#[derive(Clone)]
pub struct EtherscanAuth {
...
impl EtherscanAuth {
    ...
    ///
    /// Expects environment variable: `ETHERSCAN_API_KEY`
    ...
    ///
    /// Etherscan requires API key as a query parameter:
    /// `?apikey=YOUR_API_KEY`
...
impl Default for EtherscanAuth {
    fn default() -> Self {
        Self::from_env()
    }
}
