//! Congress.gov authentication
//!
//! Authentication type: API Key (query parameter)
//!
//! Congress.gov uses simple API key authentication via query parameter.
//! No HMAC signatures, no headers - just append `api_key=YOUR_KEY` to the URL.

use std::collections::HashMap;

/// Congress.gov authentication credentials
#[derive(Clone)]
pub struct CongressAuth {
...
impl CongressAuth {
    ...
    ///
    /// Expects environment variable: `CONGRESS_API_KEY`
    pub fn from_env() -> Self {
        Self {
            api_key: std::env::var("CONGRESS_API_KEY").ok(),
        }
    }
    ...
    ///
    /// Congress.gov requires API key as a query parameter:
    /// `?api_key=YOUR_KEY`
