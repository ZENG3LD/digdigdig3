...
//!
//! EIA uses simple API key authentication via query parameter.
//! No HMAC signatures, no headers - just append `api_key=YOUR_KEY` to the URL.

use std::collections::HashMap;

/// EIA authentication credentials
#[derive(Clone)]
pub struct EiaAuth {
...
impl EiaAuth {
    ...
    ///
    /// Expects environment variable: `EIA_API_KEY`
    pub fn from_env() -> Self {
        Self {
            api_key: std::env::var("EIA_API_KEY").ok(),
        }
    }
    ...
    ///
    /// EIA requires API key as a query parameter:
    /// `?api_key=YOUR_KEY`
