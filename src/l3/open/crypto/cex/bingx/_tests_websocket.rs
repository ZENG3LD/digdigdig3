//! BingX WebSocket Integration Tests
//!
//! Tests WebSocket connectivity and subscriptions against real BingX API.
//! BingX WebSocket messages are GZIP compressed.
//!
//! Run with:
//! ```text
//! cargo test --package digdigdig3 --lib crypto::cex::bingx::_tests_websocket -- --ignored --nocapture
//! ```
//!
//! NOTE: All tests connect to real BingX endpoints and require network access.
...
use super::websocket::BingxWebSocket;
...
async fn test_orderbook_capabilities() {
    let ws = match BingxWebSocket::new(None, false, AccountType::Spot).await {
        Ok(w) => w,
        Err(e) => {
    ...
    println!("BingX orderbook capabilities: {:?}", caps);
...
async fn test_subscribe_orderbook() {
    let mut ws = match BingxWebSocket::new(None, false, AccountType::Spot).await {
        Ok(w) => w,
        Err(e) => {
            println!("Failed to create WebSocket: {:?}", e);
            return;
    ...
    match connect_result {
        Ok(Ok(())) => {
            ...
            println!("Subscribed to BingX orderbook — waiting for snapshot...");
            ...
            }).await.expect("Timeout waiting for orderbook data — BingX did not send snapshot/delta within 15s");
            ...
            let _ = ws.disconnect().await;
            println!("BingX orderbook subscription works");
        }
...
async fn test_subscribe_trades() {
    ...
    match connect_result {
        Ok(Ok(())) => {
            ...
            println!("Subscribed to BingX trades — waiting for trade...");
            ...
            let _ = ws.disconnect().await;
            println!("BingX trades subscription works");
        }
        Ok(Err(e)) => println!("Connection failed: {:?}", e),
        Err(_) => println!("Connection timeout"),
    }
