//! Gate.io WebSocket Integration Tests
//!
//! Tests WebSocket connectivity and subscriptions against real Gate.io V4 API.
//!
//! Run with:
//! ```text
//! cargo test --package digdigdig3 --lib crypto::cex::gateio::_tests_websocket -- --ignored --nocapture
//! ```
//!
//! NOTE: All tests connect to real Gate.io endpoints and require network access.
...
use super::websocket::GateioWebSocket;
...
fn btc_usdt() -> Symbol {
    // Gate.io uses BTC_USDT format (handled internally)
    Symbol::new("BTC", "USDT")
}
...
async fn test_orderbook_capabilities() {
    let ws = match GateioWebSocket::new(None, false, AccountType::Spot).await {
        Ok(w) => w,
        Err(e) => {
...
async fn test_subscribe_orderbook() {
    let mut ws = match GateioWebSocket::new(None, false, AccountType::Spot).await {
        Ok(w) => w,
        Err(e) => {
            println!("Failed to create WebSocket: {:?}", e);
            return;
    ...
    match connect_result {
        Ok(Ok(())) => {
            ...
            println!("Subscribed to Gate.io orderbook depth=20 — waiting for snapshot...");
            ...
            }).await.expect("Timeout waiting for orderbook data — Gate.io did not send snapshot/delta within 15s");
            ...
            let _ = ws.disconnect().await;
            println!("Gate.io orderbook subscription works");
        }
...
async fn test_subscribe_trades() {
    ...
    match connect_result {
        Ok(Ok(())) => {
            ...
            println!("Subscribed to Gate.io trades — waiting for trade...");
            ...
            let _ = ws.disconnect().await;
            println!("Gate.io trades subscription works");
        }
        Ok(Err(e)) => println!("Connection failed: {:?}", e),
        Err(_) => println!("Connection timeout"),
    }
...
async fn test_orderbook_depth_levels() {
    // Gate.io valid depths: 20, 50
    let depths = [20u32, 50];

    for depth in &depths {
        println!("Testing Gate.io depth={}...", depth);
