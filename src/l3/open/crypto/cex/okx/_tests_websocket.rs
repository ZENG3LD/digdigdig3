...
//!
//! Tests WebSocket connectivity and subscriptions against real OKX API.
//!
//! Run with:
//! ```text
//! cargo test --package digdigdig3 --lib crypto::cex::okx::_tests_websocket -- --ignored --nocapture
//! ```
//!
//! NOTE: All tests connect to real OKX endpoints and require network access.
...
use super::websocket::OkxWebSocket;
...
fn btc_usdt() -> Symbol {
    ...
    Symbol::with_raw("BTC", "USDT", "BTC-USDT".to_string())
}
...
async fn test_subscribe_orderbook() {
    ...
    match connect_result {
        Ok(Ok(())) => {
            ...
            println!("Subscribed to OKX orderbook depth=5 — waiting for snapshot...");
            ...
            }).await.expect("Timeout waiting for orderbook data — OKX did not send snapshot/delta within 15s");
            ...
            let _ = ws.disconnect().await;
            println!("OKX orderbook subscription works");
        }
...
async fn test_subscribe_trades() {
    ...
    match connect_result {
        Ok(Ok(())) => {
            ...
            println!("Subscribed to OKX trades — waiting for trade...");
            ...
            let _ = ws.disconnect().await;
            println!("OKX trades subscription works");
        }
        Ok(Err(e)) => println!("Connection failed: {:?}", e),
        Err(_) => println!("Connection timeout"),
    }
