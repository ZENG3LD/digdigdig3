//! Upbit WebSocket Integration Tests
//!
//! Tests WebSocket connectivity and subscriptions against real Upbit API.
//! Upbit uses INVERTED symbol format: QUOTE-BASE (e.g., USDT-BTC, not BTC-USDT).
//!
//! Run with:
//! ```text
//! cargo test --package digdigdig3 --lib crypto::cex::upbit::_tests_websocket -- --ignored --nocapture
//! ```
//!
//! NOTE: All tests connect to real Upbit (Singapore) endpoints and require network access.
...
use super::websocket::UpbitWebSocket;
...
fn btc_usdt() -> Symbol {
    // Upbit format: USDT-BTC (QUOTE-BASE, inverted!)
    // We pass base=USDT, quote=BTC so the connector formats it as USDT-BTC
    // Actually pass it as BTC/USDT and let the connector format internally
    Symbol::new("BTC", "USDT")
}
...
async fn test_orderbook_capabilities() {
    let ws = match UpbitWebSocket::new(None, "sg").await {
        Ok(w) => w,
        Err(e) => {
    ...
    println!("Upbit orderbook capabilities: {:?}", caps);
...
async fn test_subscribe_orderbook() {
    ...
    let connect_result = timeout(Duration::from_secs(15), ws.connect(AccountType::Spot)).await;

    match connect_result {
        Ok(Ok(())) => {
            assert_eq!(ws.connection_status(), ConnectionStatus::Connected);

            // Upbit does not use depth parameter
            ...
            println!("Subscribed to Upbit orderbook (USDT-BTC) — waiting for snapshot...");
            ...
            }).await.expect("Timeout waiting for orderbook data — Upbit did not send snapshot/delta within 20s");
            ...
            let _ = ws.disconnect().await;
            println!("Upbit orderbook subscription works");
        }
...
async fn test_subscribe_trades() {
    ...
    match connect_result {
        Ok(Ok(())) => {
            ...
            println!("Subscribed to Upbit trades — waiting for trade...");
            ...
            let _ = ws.disconnect().await;
            println!("Upbit trades subscription works");
        }
        Ok(Err(e)) => println!("Connection failed: {:?}", e),
        Err(_) => println!("Connection timeout"),
    }
