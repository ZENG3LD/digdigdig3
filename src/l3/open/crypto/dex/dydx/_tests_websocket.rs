//! dYdX WebSocket Integration Tests
//!
//! Tests WebSocket connectivity and subscriptions against the real dYdX v4 Indexer.
//!
//! Run with:
//! ```text
//! cargo test --package digdigdig3 --lib l3::open::crypto::dex::dydx::_tests_websocket -- --ignored --nocapture
//! ```
//!
//! NOTE: All tests connect to real dYdX Indexer WS endpoint and require network access.
//! dYdX uses perpetual futures only. Symbol format: BTC-USD (normalized by connector).
...
async fn test_orderbook_capabilities() {
    ...
    // dYdX v4_orderbook: snapshot on subscribe, then incremental deltas
